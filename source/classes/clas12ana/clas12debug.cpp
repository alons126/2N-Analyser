...
void clas12debug::fillBeforeEl(const clas12::region_part_ptr &el) {
    ...
    double chi2_ndf =
        el->trk(clas12::DC)->getChi2() / el->trk(clas12::DC)->getNDF();
    ...
    pcal_energy_b_debug->Fill(el_pcal_energy,
                              el->cal(clas12::ECIN)->getEnergy() +
                                  el->cal(clas12::ECOUT)->getEnergy());
    ...
    if (sector <= 6 && sector >= 1) {
        sf_e_debug_b[sector - 1]->Fill(el_pcal_energy,
                                       el_sf);  // 0 indexed vector
...
void clas12debug::fillAfterEl(const clas12::region_part_ptr &el) {
    ...
    pcal_energy_a_debug->Fill(el_pcal_energy,
                              el->cal(clas12::ECIN)->getEnergy() +
                                  el->cal(clas12::ECOUT)->getEnergy());
    // DEBUG plots
    if (debug_plots && sector <= 6 && sector >= 1) {
        sf_e_debug_a[sector - 1]->Fill(el->cal(clas12::PCAL)->getEnergy(),
                                       el_sf);
...
void clas12debug::fillBeforePart(const clas12::region_part_ptr &p) {
    ...
    double chi2_ndf =
        p->trk(clas12::DC)->getChi2() / p->trk(clas12::DC)->getNDF();
    ...
    if (debug_plots && (p->par()->getCharge() >= 1) && (pid != 11)) {
        if (is_cd) {
            ...
            cd_particles_b->Fill(
                p->getPhi() * 180 / pi,
                sqrt(pow(p->par()->getPx(), 2) + pow(p->par()->getPy(), 2)));

            if (pid == 2212) {
                pid_proton_chi2_cd_debug->Fill(p->par()->getChi2Pid());
                pid_proton_tof_cd_b_debug->Fill(par_mom,
                                                tof_difference(p, mass_proton));
            ...
            if (pid == 2212) {
                ...
                pid_proton_tof_fd_b_debug->Fill(par_mom,
                                                tof_difference(p, mass_proton));
...
void clas12debug::fillAfterPart(const clas12::region_part_ptr &p) {
    ...
    if (p->par()->getCharge() >= 1 && p->par()->getPid() != 11 &&
        p->par()->getPid() == 2212) {
        debugByPid(p);
        ...
        if (is_cd) {
            cd_particles_a->Fill(
                p->getPhi() * 180 / pi,
                sqrt(pow(p->par()->getPx(), 2) + pow(p->par()->getPy(), 2)));
            pid_proton_tof_cd_a_debug->Fill(par_mom,
                                            tof_difference(p, mass_proton));
        } else if (is_fd) {
            pid_proton_tof_fd_a_debug->Fill(par_mom,
                                            tof_difference(p, mass_proton));
        }
    }
}
...
double clas12debug::getSF(const clas12::region_part_ptr &p) {
    if (p->par()->getPid() == 11)
        return (p->cal(clas12::PCAL)->getEnergy() +
...
void clas12debug::fillDCdebug(const clas12::region_part_ptr &p,
                              std::vector<std::unique_ptr<TH2D> > &h) {
    h.at(0)->Fill(p->traj(clas12::DC, 6)->getX(),
                  p->traj(clas12::DC, 6)->getY());
    h.at(1)->Fill(p->traj(clas12::DC, 18)->getX(),
                  p->traj(clas12::DC, 18)->getY());
    h.at(2)->Fill(p->traj(clas12::DC, 36)->getX(),
                  p->traj(clas12::DC, 36)->getY());
}
...
void clas12debug::InitDebugPlots() {
    ...
    for (int i = 1; i <= 6; i++) {
        sf_p_debug_b.push_back(std::make_unique<TH2D>(
            Form("sf_p_debug_b_sector_%d", i),
            Form("Sampling Fraction Before Cuts Sector_%d;Momentum "
                 "(GeV/c);Sampling Fraction",
                 i),
            100, 0, 6, 100, 0, .4));
        sf_p_debug_a.push_back(std::make_unique<TH2D>(
            Form("sf_p_debug_a_sector_%d", i),
            Form("Sampling Fraction  After Cuts Sector_%d;Momentum "
                 "(GeV/c);Sampling Fraction",
                 i),
            100, 0, 6, 100, 0, .4));

        sf_e_debug_b.push_back(std::make_unique<TH2D>(
            Form("sf_e_debug_b_sector_%d", i),
            Form("Sampling Fraction Before Cuts Sector_%d;Energy "
                 "(GeV);Sampling Fraction",
                 i),
            100, 0, 1.5, 100, 0, .4));
        sf_e_debug_a.push_back(std::make_unique<TH2D>(
            Form("sf_e_debug_a_sector_%d", i),
            Form("Sampling Fraction  After Cuts Sector_%d;Energy "
                 "(GeV);Sampling Fraction",
                 i),
            100, 0, 1.5, 100, 0, .4));

        dc_edge_el_chi2_r1.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_chi2_r1_%d", i),
            Form("Region 1 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_el_chi2_r2.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_chi2_r2_%d", i),
            Form("Region 2 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_el_chi2_r3.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_chi2_r3_%d", i),
            Form("Region 3 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));

        dc_edge_p_chi2_r1.push_back(
            std::make_unique<TH1D>(Form("dc_edge_p_chi2_r1_%d", i),
                                   Form("Region 1 Protons DC edge %d;Distance "
                                        "to Edge (cm);#chi^{2}/DOF",
                                        i),
                                   100, 0, 50));
        dc_edge_p_chi2_r2.push_back(
            std::make_unique<TH1D>(Form("dc_edge_p_chi2_r2_%d", i),
                                   Form("Region 2 Protons DC edge %d;Distance "
                                        "to Edge (cm);#chi^{2}/DOF",
                                        i),
                                   100, 0, 50));
        dc_edge_p_chi2_r3.push_back(
            std::make_unique<TH1D>(Form("dc_edge_p_chi2_r3_%d", i),
                                   Form("Region 3 Protons DC edge %d;Distance "
                                        "to Edge (cm);#chi^{2}/DOF",
                                        i),
                                   100, 0, 50));

        dc_edge_el_r1.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_r1_%d", i),
            Form("Region 1 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_el_r2.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_r2_%d", i),
            Form("Region 2 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_el_r3.push_back(std::make_unique<TH1D>(
            Form("dc_edge_el_r3_%d", i),
            Form("Region 3 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));

        dc_edge_p_r1.push_back(std::make_unique<TH1D>(
            Form("dc_edge_p_r1_%d", i),
            Form("Region 1 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_p_r2.push_back(std::make_unique<TH1D>(
            Form("dc_edge_p_r2_%d", i),
            Form("Region 2 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
        dc_edge_p_r3.push_back(std::make_unique<TH1D>(
            Form("dc_edge_p_r3_%d", i),
            Form("Region 3 DC edge %d;Distance to Edge (cm);#chi^{2}/DOF", i),
            100, 0, 50));
    }
    ...
    // DC hit maps
    for (int i = 1; i <= 3; i++) {
        dc_hit_map_b.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_before_%d", i),
            Form("Region %d Before Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));
        dc_hit_map_a.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_after_%d", i),
            Form("Region %d After Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));

        dc_hit_map_a_proton.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_after_proton_%d", i),
            Form("Region %d After Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));
        dc_hit_map_b_proton.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_before_proton_%d", i),
            Form("Region %d Before Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));
        dc_hit_map_a_pion.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_after_pion_%d", i),
            Form("Region %d After Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));
        dc_hit_map_b_pion.push_back(std::make_unique<TH2D>(
            Form("dc_hitmap_before_pion_%d", i),
            Form("Region %d Before Cuts;x-position (cm);y-position (cm)", i),
            600, -300, 300, 600, -300, 300));

        //       dc_hit_map_b[i] = new TH2D(Form("dc_hitmap_before_%d",i),
        //       Form("Region %d Before Cuts",i),600,-300,300,600,-300,300);
        //       dc_hit_map_a[i] = new TH2D(Form("dc_hitmap_after_%d",i),
        //       Form("Region %d After Cuts",i),600,-300,300,600,-300,300);
        /*
        dc_hit_map_a_proton[i] = new TH2D(Form("dc_hitmap_after_proton_%d",i),
        Form("Region %d After Cuts",i),600,-300,300,600,-300,300);
        dc_hit_map_b_proton[i] = new TH2D(Form("dc_hitmap_before_proton_%d",i),
        Form("Region %d Before Cuts",i),600,-300,300,600,-300,300);
        dc_hit_map_a_pion[i] = new TH2D(Form("dc_hitmap_after_pion_%d",i),
        Form("Region %d After Cuts",i),600,-300,300,600,-300,300);
        dc_hit_map_b_pion[i] = new TH2D(Form("dc_hitmap_before_pion_%d",i),
        Form("Region %d Before Cuts",i),600,-300,300,600,-300,300);
        */
    }
}
...
void clas12debug::WriteDebugPlots(TString filename) {
    ...
    std::for_each(sf_p_debug_b.begin(), sf_p_debug_b.end(),
                  [](auto &el) { el->Write(); });
    std::for_each(sf_p_debug_a.begin(), sf_p_debug_a.end(),
                  [](auto &el) { el->Write(); });
    std::for_each(sf_e_debug_a.begin(), sf_e_debug_a.end(),
                  [](auto &el) { el->Write(); });
    std::for_each(sf_e_debug_b.begin(), sf_e_debug_b.end(),
                  [](auto &el) { el->Write(); });

    std::for_each(dc_edge_el_chi2_r1.begin(), dc_edge_el_chi2_r1.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_el_chi2_r2.begin(), dc_edge_el_chi2_r2.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_el_chi2_r3.begin(), dc_edge_el_chi2_r3.end(),
                  [](auto &p) { p->Write(); });

    std::for_each(dc_edge_el_r1.begin(), dc_edge_el_r1.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_el_r2.begin(), dc_edge_el_r2.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_el_r3.begin(), dc_edge_el_r3.end(),
                  [](auto &p) { p->Write(); });

    std::for_each(dc_edge_p_chi2_r1.begin(), dc_edge_p_chi2_r1.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_p_chi2_r2.begin(), dc_edge_p_chi2_r2.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_p_chi2_r3.begin(), dc_edge_p_chi2_r3.end(),
                  [](auto &p) { p->Write(); });

    std::for_each(dc_edge_p_r1.begin(), dc_edge_p_r1.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_p_r2.begin(), dc_edge_p_r2.end(),
                  [](auto &p) { p->Write(); });
    std::for_each(dc_edge_p_r3.begin(), dc_edge_p_r3.end(),
                  [](auto &p) { p->Write(); });
