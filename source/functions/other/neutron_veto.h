...
bool NeutronCD_CutVeto(const std::unique_ptr<clas12::clas12reader>& c12, int index, int cnd_hits, int cnd_hit_layer, int cnd_hit_sector, int cnd_hit_energy, int ctof_hits, int ctof_hit_component, int ctof_hit_energy)
{
  auto allParticles = c12->getDetParticles();
  ...
  for(int k = 0; k < allParticles.size(); k++){
    ...
    int vetoSectorbyLayer[4] = {(allParticles[k]->sci(clas12::CTOF)->getComponent()+1)/2,allParticles[k]->sci(clas12::CND1)->getSector(),allParticles[k]->sci(clas12::CND2)->getSector(),allParticles[k]->sci(clas12::CND3)->getSector()};
    for(int k = 0; k < 4; k++){	    
...
bool NeutronECAL_CutVeto(const std::unique_ptr<clas12::clas12reader>& c12, double Ebeam, int index){
  ...
  for(int j = 0; j < allParticles.size(); j ++){
	...
	Veto=true;
      ...
      v_chit.SetXYZ(allParticles[j]->traj(clas12::ECAL,trajlayer)->getX(),allParticles[j]->traj(clas12::ECAL,trajlayer)->getY(),allParticles[j]->traj(clas12::ECAL,trajlayer)->getZ());
      TVector3 v_dist=v_nhit-v_chit;
      if(v_dist.Mag()<100){
	Veto=true;
      }
